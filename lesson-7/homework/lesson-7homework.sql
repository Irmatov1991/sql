...
select MAX(price) as price_min from Products
...
select SUM(saleamount) as amount_total from Sales
where ProductID = 7

select AVG(age) as age_avarega
from employees

select employeeid, COUNT(*) as emp_num
from employees
group by employeeid
...
select ProductID,
	...
	AVG(saleamount) as sale_avarage
	from Sales
	group by ProductID

select COUNT(*) as emp_total
	from employees
...
select departmentname,
	avg(salary) as salary_avarage,
	count(*) as count_emp
	from employees
	group by DepartmentName

select category,
	AVG(price) as price_avarage
	from Products
	group by Category
...
SELECT 
    ...
    MIN(od.qty) AS min_qty,
    MAX(od.qty) AS max_qty
FROM Sales.Orders AS o
JOIN Sales.OrderDetails AS od
    ON o.orderid = od.orderid
