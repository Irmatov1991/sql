...
select 
		CAST(employee_id as varchar(10)) + '-' + FIRST_NAME + '-' + LAST_NAME as employee_details 
		from Employees
...
SELECT 
    id,
    ISNULL(ssn, ISNULL(passportid, itin)) AS first_non_null_value
FROM person;
...
parsename(replace(fullname,' ', '.'),1) as lasrname
from Students
...
SELECT 
   job_id,
    ...
    ROUND(
        100.0 * AVG(CASE WHEN DATEDIFF(YEAR, hire_date, GETDATE()) > 3 THEN 1.0 ELSE 0 END),
        2
...
select StudentID, FullName, grade,
	sum(grade) over (order by studentid rows between unbounded preceding and current row) 
as runing_total 
from Students
order by StudentID
...
select Birthday,
	String_agg(studentname, ',') as studentwithname,
	count(*) as student_birthday
	from Student
	group by birthday
	having count(*) >1
	order by Birthday
...
SELECT 
    ...
    -- Extract Uppercase
    REPLACE(@str, TRANSLATE(@str, 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', REPLICATE('#', 26)), '') AS UppercaseLetters,
    -- Extract Lowercase
    REPLACE(@str, TRANSLATE(@str, 'abcdefghijklmnopqrstuvwxyz', REPLICATE('#', 26)), '') AS LowercaseLetters,
    -- Extract Numbers
    REPLACE(@str, TRANSLATE(@str, '0123456789', REPLICATE('#', 10)), '') AS Numbers,
    -- Extract Others
    REPLACE(@str, TRANSLATE(@str, 'abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789', REPLICATE('#', 62)), '') AS SpecialCharacters;
