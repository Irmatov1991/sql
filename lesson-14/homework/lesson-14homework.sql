...
select ID,
	parsename(replace(vals,'.','.'),1) as part1,
	parsename(replace(vals,'.','.'),2) as part2,
	parsename(replace(vals,',','.'),3) as part3
	from Splitter
...
--5--
select texts,
	(len(texts) - len(replace(texts, ' ', ''))) as countspace from CountSpaces
...
datediff(year,hire_date,getdate()) as hireyear 
from employees
...
SELECT 
    LTRIM(RTRIM(value)) AS third_item
FROM STRING_SPLIT(
...
select EMPLOYEE_ID, FIRST_NAME,LAST_NAME, hire_date,
datediff(year,hire_date,getdate()) as year_service,
case
	when datediff(year,Hire_date,getdate()) < 1 then 'newhire'
	when datediff(year,Hire_date,getdate()) between 1 and 5 then 'junior'
	when datediff(year,Hire_date,getdate()) between 6 and 10 then 'middle'
	when datediff(year,Hire_date,getdate()) between 11 and 20 then 'senior'
	else 'veteran'
	end as year_emp
	from employees
	order by year_service desc

--12--

select Vals,
left(vals,patindex('%[0-9]%',vals + 'a') - 1) as startinger
from MultipleVals
where Vals like '[0-9]%'
...
SELECT 
    player_id,
    device_id AS first_logged_device
FROM (
    SELECT 
        player_id,
        device_id,
...
SELECT 
    ...
    REGEXP_REPLACE('rtcfvty34redt', '[^0-9]', '') AS Integers;
