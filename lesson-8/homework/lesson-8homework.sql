...
select category,
count(category) as total_number
from Products
group by Category
...
else 'Sufficient' end as stock_quatity
from Products
...
select distinct o.CustomerID,Inv.InvoiceDate, inv.TotalAmount from orders as o
left join Invoices as inv
on o.OrderID = inv.CustomerID
...
FROM city_population
UNPIVOT (
    city_population FOR districtname IN ([Bektemir], [Chilonzor], [Yakkasaroy])
) AS unpvt;
