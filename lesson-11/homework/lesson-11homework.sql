...
select o.CustomerID, c.firstname, o.OrderDate from Orders as o
join Customers as c
on c.CustomerID = o.CustomerID
where OrderDate > '2022-12-21'
...
select d.DepartmentName, MAX(e.salary) as max_sal from Departments as d
join Employees as e
...
where e.DepartmentID = d.DepartmentID) as max_salary
...
and YEAR(o1.OrderDate) = 2023)

--5--

select c.firstname, count(o.orderid) as totalord from Customers as c
...
select c.FirstName, MAX(o.orderdate) as maxrecentdateorderdate from Customers as c
left join Orders as o
on c.CustomerID = o.CustomerID
group by c.FirstName
...
--8--
select c.FirstName, o.TotalAmount as ordertotal from Orders as o
join Customers as c
on o.CustomerID = c.CustomerID
where o.TotalAmount > 500 

select c.FirstName from Customers as c
where exists (select o.totalamount as totalorder from Orders as o
...
select e.Name, (select d.DepartmentName from Departments as d
where e.DepartmentID = d.DepartmentID) as departamentname, 
e.Salary from Employees as e
where e.Salary > 60000
and e.DepartmentID in (select 
...
select 
	...
	e.HireDate from Employees as e
Join Departments as d
	on e.DepartmentID = d.DepartmentID
where YEAR(e.hiredate) > 2020
	or d.DepartmentName in ('Sales')
...
WHERE EXISTS (
    SELECT 1
    FROM Products p2
    WHERE p2.ProductID = s.ProductID
      AND (p2.Category = 'Electronics' OR s.SaleAmount > 350)
);
...
WHERE EXISTS (
    ...
    WHERE d2.DepartmentID = e.DepartmentID
      AND d2.DepartmentName IN ('Sales', 'Marketing')
)
AND e.Salary > 60000;
